...
LLImmlev.basic<-function(status.matrix.v2,infectee,lli,current.time,typeIC,t.imm.lim,pathogen1,pathogen2, status.matrix.v1){ #pathogen.v1 is the infection the infectee might catch
  value<-1
  
  if (pathogen1=="DELTA" & pathogen2=="OMICRON"){
        if (pathogen1 =="DELTA"){
          if (status.matrix.v1$Immunity[infectee]==1){
            if (status.matrix.v1$infected[infectee]==0 | status.matrix.v1$infected[infectee]==-1){
              value<-VE.COVID()
...
sim.multipathogen.variants<-function(HH.network, t2, lambda.g, sigma21, sigma12, prop.immune, nSeeds.1,nSeeds.2, rho.1,rho.2,inf.path.1.h,inf.path.1.g,inf.path.2.h,inf.path.2.g, alpha.as.1,alpha.as.2,lli.1,lli.2, pathogen.1,pathogen.2, contact.reduction,t.stop,t.seed, bc.1,bc.2,reinf,typeIC, het.vac, t.imm.lim){
  ...
  while((sum(infectives)>0 & current.time<t.stop) | current.time<t2){ #while there are still infectives
    ...
    if (next.evts=="NextCtc"){
      ...
      if (status.matrix.1$infected[infector]==1 & status.matrix.1$infected[infectee]==0){
        ...
        long.inter<-LLImmlev.basic(status.matrix.v2 = status.matrix.2,infectee = infectee, lli = lli.2, current.time = current.time,typeIC = typeIC, t.imm.lim = t.imm.lim, pathogen1 = pathogen.1, pathogen2 = pathogen.2, status.matrix.v1 = status.matrix.1 )
      ...
      if (status.matrix.2$infected[infector]==1 & status.matrix.2$infected[infectee]==0){
        ...
        long.inter<-LLImmlev.basic(status.matrix.v2 = status.matrix.1,infectee = infectee, lli = lli.1, current.time = current.time,typeIC = typeIC, t.imm.lim = t.imm.lim, pathogen1 = pathogen.2, pathogen2 = pathogen.1, status.matrix.v1 = status.matrix.2)
